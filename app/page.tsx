  ...
  return (
    <main className="mx-auto max-w-4xl p-8">
      ...
      <section className="mt-10">
        <h2 className="mb-3 text-sm font-medium uppercase tracking-wide text-neutral-500">Projects</h2>
        {projects.length === 0 ? (
          ...
          <ul className="divide-y divide-neutral-200 rounded-lg border border-neutral-200">
            {projects.map((p) => (
              <li key={p.id}>
                <Link
                  href={`/projects/${p.id}`}
                  className="flex items-center justify-between p-4 transition-all active:scale-[0.99] hover:bg-neutral-50"
                >
                  <span>
